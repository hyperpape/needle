...
class NFAToDFACompilerTest {
    ...
    void compileBasic() {
        ...
        var dfa = new NFAToDFACompiler(nfa)._compile(ConversionMode.BASIC);
    ...
    void compileContainedIn() {
        var nfa = NFA.createNFANoAhoCorasick("(AB){1,2}");
        var dfa = new NFAToDFACompiler(nfa)._compile(ConversionMode.CONTAINED_IN);
    ...
    void compileDFASearch() {
        var nfa = NFA.createNFANoAhoCorasick("(AB){1,2}");
        var dfa = new NFAToDFACompiler(nfa)._compile(ConversionMode.DFA_SEARCH);
