...
class NFAToDFACompilerTest {
    ...
    void compileBasic() {
        ...
        var dfa = new NFAToDFACompiler(nfa, false)._compile(nfa, ConversionMode.BASIC);
    ...
    void compileContainedIn() {
        var nfa = NFA.createNFANoAhoCorasick("(AB){1,2}");
        var dfa = new NFAToDFACompiler(nfa, false)._compile(nfa, ConversionMode.CONTAINED_IN);
    ...
    void compileDFASearch() {
        var nfa = NFA.createNFANoAhoCorasick("(AB){1,2}");
        var dfa = new NFAToDFACompiler(nfa, false)._compile(nfa, ConversionMode.DFA_SEARCH);
